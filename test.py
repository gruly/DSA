...
from data.utils import convert_to_NE 
from model_seq2seqAtt import DeepSpeech
import subprocess
...
#	c	    print " audio_ids ", audio_ids[x], " x j ", x, " ",j, " score  ",scores[x][j], " utterance  ", decoded_output[x][j]
    ...
    if decoder is not None:
        if args.eval == 'concept':
            wer=n_concept_err/float(n_concept_ref)
        else:
	...
	if args.eval=='concept': 
            print('Test Summary \t'
              ...
              'Average CER {cer:.3f}\t'.format(Conper=float(wer), cer=cer * 100))
	else:
	    print('Test Summary \t'
              'Average Word Error Rate {wer:.3f}\t'
