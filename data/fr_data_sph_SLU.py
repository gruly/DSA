...
parser = argparse.ArgumentParser(description='Processes French SLU dataset :  change the tags to NE format :  from word:tag to <tag word >.')
...
parser.add_argument('--starred_mode', dest='starred_mode', action='store_true', help='Turn on starred mode option : keep only the SLU/NE tags ans the values and replace the other words to * ')
...
def _preprocess_transcript(phrase):
	...
	if ":" in words[i] :
	    ...
	    new_phrase=new_phrase+"<"+re.sub("-[B,I]$","",word_tag[1])+" "+word_tag[0]+" > "
	    
	else:
	    new_phrase=new_phrase+words[i]+" "

    return new_phrase.strip().lower().encode("utf-8", "ignore").decode("utf-8", "ignore")
...
def prepare_dir(target_dir, source_dir):
    ...
    for sph_file in tqdm(entries, total=len(entries)):
        ...
        stm_file_full = os.path.join(source_dir, "stm", "{}.stm.align.align".format(speaker_name))
...
def main():
    ...
    prepare_dir(target_train_dir, source_train) 
    print " prepare data for dev  "
    prepare_dir(target_val_dir, source_val)
    print " prepare data for test  "
    prepare_dir(target_test_dir, source_test)
