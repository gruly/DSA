...
def cut_utterance(src_sph_file, target_sph_file, start_time, end_time, sample_rate=16000):
    #print " src_sph_file ", src_sph_file, " start_time ", start_time,  "  end_time  ", end_time
    subprocess.call(["sox {}  -r {} -b 16 -c 1 {} trim {} ={}".format(src_sph_file, str(sample_rate),
                                                                      target_sph_file, start_time, end_time)],
                    shell=True)
...
def prepare_dir(target_dir, source_dir,data_type):
    ...
    for sph_file in tqdm(entries, total=len(entries)):
        ...
        stm_file_full = os.path.join(source_dir,"stm_norm",data_type, "{}.stm".format(speaker_name))
...
#        print " wav_file_full  ", wav_file_full
	...
	if get_duration(wav_file_full)  != get_duration(sph_file_full) :
               ...
               print " durations  ", " wav ", get_duration(wav_file_full) , " sph ",get_duration(sph_file_full)
               print " bad audio ", wav_file_full 
	    ...
	    print " stm_file_full, ",stm_file_full
            ...
            for utterance_id, utterance in enumerate(all_utterances):
                target_sph_file = os.path.join(wav_dir, "{}_{}.wav".format(utterance["filename"], str(utterance_id)))
                target_txt_file = os.path.join(txt_dir, "{}_{}.txt".format(utterance["filename"], str(utterance_id)))
                cut_utterance(sph_file, target_sph_file, utterance["start_time"], utterance["end_time"],
	    ...
	    print " prob stm  or audio  stm_file_full ", stm_file_full, " sph_file " , sph_file
def main():
...
#    prepare_dir(target_val_dir, source_val, data_type)
 ...
 #   prepare_dir(target_test_dir, source_test, data_type)
    ...
    create_manifest(target_dl_dir, target_train_dir, 'train',data_type)
  #  create_manifest(target_dl_dir, target_val_dir, 'val',data_type)
  #  create_manifest(target_dl_dir, target_test_dir, 'test', data_type)
