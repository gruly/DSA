...
def create_manifest(target_dl_dir, data_path, tag, data_type="", ordered=True):
    manifest_path = '%s%s_%s_manifest_ne_augment_CL_data_.csv' % (target_dl_dir,data_type,tag)
...
def main():
  ...
  #  prepare_dir(target_test_dir, source_test, data_type)
    ...
    print " target_dl_dir  ", target_dl_dir
    create_manifest(target_dl_dir, os.path.join(target_train_dir, "converted_aug_CL"), 'train',data_type)
