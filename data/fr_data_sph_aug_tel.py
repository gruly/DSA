...
parser = argparse.ArgumentParser(description='Processes telphon  French data .')
...
#        wav_file_full = os.path.join(source_dir, "wav", "{}.wav".format(speaker_name))
        ...
        if os.path.exists(sph_file_full) == True and  os.path.exists(stm_file_full) == True :
	      ...
	      last_duration=0
       	   if float(get_duration(sph_file_full)) + 0.10 >= last_duration :
	     # print " stm_file_full, ",stm_file_full
              ...
              for utterance_id, utterance in enumerate(all_utterances):
                ...
                target_wav_file = os.path.join(wav_dir, "{}_{}.wav".format(speaker_name, str(utterance_id)))
                #target_txt_file = os.path.join(txt_dir, "{}_{}.txt".format(utterance["filename"], str(utterance_id)))
                target_txt_file = os.path.join(txt_dir, "{}_{}.txt".format(speaker_name, str(utterance_id)))
...
def main():
    ...
    # prepare target dir
    target_t1_dir = os.path.join(target_dl_dir, "new_train","1")
    if not os.path.exists(target_t1_dir):
        os.makedirs(target_t1_dir)
    target_t2_dir = os.path.join(target_dl_dir, "new_train","2")
    if not os.path.exists(target_t2_dir):
        os.makedirs(target_t2_dir)
    target_t3_dir = os.path.join(target_dl_dir, "new_train","3")
    if not os.path.exists(target_t3_dir):
        os.makedirs(target_t3_dir)
    target_t4_dir = os.path.join(target_dl_dir, "new_train","4")
    if not os.path.exists(target_t4_dir):
        os.makedirs(target_t4_dir)

    # source dir
    source_t1 = os.path.join(source_dl_dir, "new_train","1")
    source_t2 = os.path.join(source_dl_dir, "new_train","2")
    source_t3 = os.path.join(source_dl_dir, "new_train","3")
    source_t4 = os.path.join(source_dl_dir, "new_train","4")
 
    print " prepare data for train  "
    prepare_dir(target_train_dir, source_t1, data_type) 
    prepare_dir(target_train_dir, source_t2, data_type) 
    prepare_dir(target_train_dir, source_t3, data_type) 
    prepare_dir(target_train_dir, source_t4, data_type) 
 ...
 #   prepare_dir(target_val_dir, source_val, data_type)
  ...
  #  prepare_dir(target_test_dir, source_test, data_type)
    ...
    create_manifest(target_dl_dir, os.path.join(target_t1_dir, "converted"), 'new_train',data_type)
