...
def decode_dataset(logits, test_dataset, batch_size, lm_alpha, lm_beta, mesh_x, mesh_y, index, labels, eval):
    ...
    for i, (data) in enumerate(test_loader):
        ...
        for x in range(len(target_strings)):
            ...
            if eval == 'concept':
		ref_file.write(convert_to_NE(reference).encode('utf-8')+"("+audio_ids[x]+")\n")
                trans_file.write(convert_to_NE(transcript).encode('utf-8')+"("+audio_ids[x]+")\n")
    ...
    if eval == 'concept': # Concept error rate evaluation  	
   	cmd="perl /lium/buster1/ghannay/deepSpeech2/deepspeech.pytorch/data/eval.sclit_cer2.pl %s"%(eval_dir)
	print ("cmd  ",cmd)
