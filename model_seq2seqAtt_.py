...
class DeepSpeech(nn.Module):
	...
	# Decoder
        ...
        self.attn = Attn('concat', self._hidden_size, self._dec_hidden_size)
  ...
  #      print (" y.size  ", y.size())       
	...
	# TODO il faut changer l'initialisation de la couche de decoder avec la dernier couche de l'encoder
       # print (" decoder_hidden  ", type(decoder_hidden), "  decoder_hidden  ", decoder_hidden.size())
        ...
        encoder_outputs=x
   #     print (" encoder_outputs  x  ", encoder_outputs.size())
...
#	print ("  len( y ", len(y) )
	    ...
	    # decoder
            decoder_input = y[t]
            print (" decoder_input  ", decoder_input.is_cuda)
#            print (" t  ", t)
 #           print ( " -->    decoder_input.sie ", decoder_input.size())
            last_hidden=decoder_hidden
            print (" last_hidden  ", last_hidden.is_cuda)
#	    print("last_hidden size",last_hidden.size())
            ...
            print (" word_embedded  ", word_embedded.is_cuda)
 #           print (" inputs embed  size  ", word_embedded.size())
	    word_embedded = self.dropout(word_embedded)
    # Calculate attention weights and apply to encoder outputs
  #          print (" compute attention ")
            attn_weights = self.attn(last_hidden, encoder_outputs)
            context = attn_weights.bmm(encoder_outputs.transpose(0, 1)) # B x 1 x N
	    print ( " attn weights  size  ", attn_weights.is_cuda)
            context = context.transpose(0, 1) # 1 x B x N
 #           print ( " context  size  ", context.size())
            
            # Combine embedded input word and attended context, run through RNN
            rnn_input = torch.cat((word_embedded, context), 2)
            print ( " rnn_input  ", rnn_input.is_cuda)
            decoder_output, decoder_hidden = self.gru(rnn_input, last_hidden)
            # Final output layer
            print (" decoder_output  ", decoder_output.is_cuda)
            context = context.squeeze(0) # B x N
            print (" context squeeze(0)   ", context.is_cuda)
            decoder_output = decoder_output.squeeze(0) # B x N
            print (" decoder_output squeeze(0)   ", decoder_output.is_cuda)
            decoder_output = torch.cat((decoder_output, context), 1)
            print (" torch.cat((decoder_output, context), 1)   ",decoder_output.is_cuda) 
            decoder_output = self.fc(decoder_output)
            print (" self.fc(decoder_output)   ", decoder_output.is_cuda) 
            decoder_output = self.inference_softmax(decoder_output)
            print (" self.inference_softmax(decoder_output)    ", decoder_output.is_cuda) 
...
class Attn(nn.Module):
    def __init__(self, method, enc_hidden_size, dec_hidden_size):
        ...
        self.method = method
        self.hidden_size = enc_hidden_size+dec_hidden_size
...
#	print ("ATTN FORWARD : hidden.size()", hidden.size(), "encoder_output.size()", encoder_outputs.size())
        max_len = encoder_outputs.size(0)
 #       print (" max_len  ", max_len)
        this_batch_size = encoder_outputs.size(1)
  #      print (" this_batch_size    ", this_batch_size)
        # Create variable to store attention energies
        attn_energies = Variable(torch.zeros(this_batch_size, max_len)) # BxS
